      ...
      {deferredPrompt && !dismissed && !installed && (
        <>
          <motion.div
            ...
            exit={{ opacity: 0, y: 18 }}
            className="fixed bottom-[calc(6.2rem+env(safe-area-inset-bottom))] right-4 z-50 sm:hidden"
          >
            <div className="flex items-center gap-2 rounded-full border border-white/40 bg-white/90 px-3 py-2 text-xs shadow-neu-sm backdrop-blur-xl dark:border-white/10 dark:bg-white/10">
              <button onClick={() => void install()} className="inline-flex items-center gap-1.5 font-medium text-ink">
                <Download className="h-3.5 w-3.5" />
                Add app
              </button>
              <button
                onClick={() => setDismissed(true)}
                className="rounded-full p-1 text-ink/45 transition hover:bg-white/40 hover:text-ink"
          ...
          <motion.div
            ...
            className="fixed inset-x-4 bottom-[calc(6.5rem+env(safe-area-inset-bottom))] z-50 mx-auto hidden max-w-sm sm:block"
          >
